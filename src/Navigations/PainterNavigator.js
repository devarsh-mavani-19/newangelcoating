...
import TransactionQR from '../Painter/Transaction_QrCodes'
...
const Tabs = {
  Home: {
    ...
    navigationOptions: () => ({
      tabBarIcon: ({ tintColor }) => {
        return <AntDesign name='home' size={hp('4%')} color={tintColor} />
      }
    })
  },
  ...
  Profile: {
    ...
    navigationOptions: () => ({
      tabBarIcon: ({ tintColor }) => {
        return <AntDesign name='user' size={hp('4%')} color={tintColor} />
      }
    })
  }
}

const PainterScreens = createBottomTabNavigator(Tabs, {
  tabBarOptions: {
