...
export default class Demo extends React.Component {
          ...
          if (result.docs[0].data()['account'] == 'Seller') {
            ...
            if (result.docs[0].data()['isApproved'] === true) {
              ...
              that.setState({
                notApproved: true,
                UserName:
                  result.docs[0].data()['firstName'] +
                  ' ' +
                  result.docs[0].data()['lastName'],
                Email: result.docs[0].data()['email'],
                Image: result.docs[0].data()['Profile_Image'],
                Phone: result.docs[0].data()['phone'],
                SellerCode: result.docs[0].data()['code'],
  ...
  updaterequest = async () => {
    ...
    await firestore()
      .collection('users')
      .where('code', '==', parseInt(this.state.editsellecode))
      .get()
      .then(data => {
  ...
  resned = async () => {
    ...
    if (this.state.Deny === false) {
      this.setState({ process2: 0 })
      alert('You can not send request! now')
    } else {
      const ref = firestore()
        .collection('Seller_Request')
        .doc(this.state.requestid)
        ...
        {this.state.notApproved === true ? (
          <View>
            ...
            <View style={styles.main}>
              <View style={{ marginTop: hp('2%') }}>
                  ...
                  {this.state.Deny === true ? (
                    ...
                    >
                      <Text
                        style={{
                          ...styles.text,
                          ...{ fontSize: hp('2.5%'), color: 'red' }
                        }}
                      >
                        {this.state.account === 'Painter' ? 'Seller' : 'Admin'}{' '}
                        Denied Your Request
                      </Text>
                    </View>
                  ) : (
                ...
                >
                  ...
                  {this.state.account === 'Painter' ? (
                    ...
                    <TouchableNativeFeedback onPress={() => this.resned()}>
                      <Card
                        style={{
                          height: hp('6%'),
                          width: wp('35%'),
              ...
              <Loading isVisible={this.state.process > 0} data='Editing' />

              <Loading isVisible={this.state.process2 > 0} data='Sending' />
                  ...
                  <View style={{ flexDirection: 'row' }}>
                    ...
                    <View>
                      <TextInput
                        ...
                        onChangeText={text =>
                          this.setState({ editsellecode: text })
                        }
                      />
                    </View>
                  </View>
